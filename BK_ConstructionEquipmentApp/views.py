...
class PendingSaveService(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        #userService = ServiceAssigned.objects.filter(To=User[0], EntryDate__range=(sDate, eDate)).order_by('-id').values('ServiceId').using('MotorConstructionEquipment')
        userService = []
        allService = []
        print('----' + str(User))
        if str(userId) == 'admin':
            ...
            #allSer = ServiceAssigned.objects.filter(From=User[0], EntryDate__range=(sDate, eDate)).values('ServiceId').order_by('-id').using('MotorConstructionEquipment')
            #service = Service.objects.filter(EntryBy = User[0], EntryDate__range=(sDate, eDate)).values('id').using('MotorConstructionEquipment')
            #engspeService = ServiceAssigned.objects.filter(ServiceId__in=service).values('ServiceId').order_by('-id').using('MotorConstructionEquipment')
            
            #mergedlist = list(allSer) + list(engspeService)
            ...
            #allServiceCre = ServiceAssigned.objects.filter(ServiceId__in=engspeService).values('ServiceId', 'ServiceId__CompanyName', 'To__UserName', 'ServiceId__ContactPerson', 'ServiceId__ServiceTime', 'Status','To__RoleId__id','color', 'To').using('MotorConstructionEquipment')
            #allServiceCre = allServiceCre.extra(select={'datestr': "to_char(ServiceId__ServiceTime, 'YYYY-MM-DD HH24:MI:SS')"})
            #allService = list(allServiceTo) + list(allServiceCre)
            
            #allSerList = allService.extra(select={'datestr': "to_char(ServiceId__ServiceTime, 'YYYY-MM-DD HH24:MI:SS')"})
