...
class PiImageSave(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        sms = 'Dearest Customer, we are sorry to inform you that your Yamaha Booking Bike will be delivered within 20th to 25th February, 2019. For any inquiry - 01704123991'
        #text = 'UserId : ' + user['UserId'] + " Passwors : " + user['Password']
        #SendSMS('01920250777', sms)
    ...
    def create(self, request):
        ...
        for item in json_array:
            ...
            if str(collectiondate) == '':
                query = "INSERT  INTO CollMasterNew(Amount, CustomerCode, mrn, CreateDate, StuffId, SupportedBy, Remarks, Latitude, Longitude) VALUES(" + str(
                    amount) + ", '" + customerCode + "', '" + str(
                    mrn) + "', '" + created_at + "', '" + stuffId + "','"+supportedby+"','"+remarks+"',"+latitude+","+longitude+")"
            else:
                query = "INSERT INTO CollMasterNew(Amount, CustomerCode, mrn, CollectionDate, CreateDate, StuffId, SupportedBy, Remarks, Latitude, Longitude) VALUES(" + str(
                    amount) + ", '" + customerCode + "', '" + str(
                    mrn) + "', '" + collectiondate + "', '" + created_at + "', '" + stuffId + "','"+supportedby+"','"+remarks+"',"+latitude+","+longitude+")"
        ...
        for item in projection_array:
            ...
            query = query = "INSERT  INTO ProjectionMaster(StaffID, CustomerCode, ProjectionAmount, ProjectionDate, CustomerMobile, CreateDate) VALUES('" + str(StaffID) + "', '" + CustomerCode + "', '" + ProjectionAmount + "', '" + ProjectionDate + "', '" + CustomerMobile + "', '" + CreateDate + "')"
            print(query)
            cursor.execute(query)
        ...
        for item in capture_array:
            ...
            query = query = "INSERT  INTO CaptureMaster(StaffID, CustomerCode, CustomerName, CaptureDate, CaptureLocation, CaptureTractorModel, CaptureOther) VALUES('" + str(
        ...
        for item in release_array:
            ...
            amount = item['amount']
            query = query = "INSERT  INTO ReleaseMaster(StaffID, CustomerCode, CustomerName, ReleaseDate, Amount) VALUES('" + str(
...
class MyAppCustomer(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        sqlst = "SELECT aum.UserName, cas.Code, cas.CustomerName, ForMonth, '0' AS SyncStatus FROM AppUserManager aum INNER JOIN Territory TT ON aum.TerritoryCode = TT.TTYCode INNER JOIN CreditAnalysisStg cas ON cas.Territory LIKE TT.TTYName WHERE (cas.ForMonth = '2019-03-01') AND aum.UserName = '"+str(userId)+"'"
        print('--'+str(sqlst))
