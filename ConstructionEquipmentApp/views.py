...
class PendingSaveService(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if str(userId) == 'admin':
            ...
            serId = temp[0]['ServiceId']
            for it in reversed(temp):
                #serId = it['ServiceId']
                if serId != it['ServiceId']:
                    showLis.append(it)
                serId = it['ServiceId']
    ...
    def list(self, request):
        ...
        if str(entryBy) == 'admin':
            #assignedPerson = ServiceAssigned.objects.filter(ServiceId=seId[0]).values('To__UserName','To__RoleId__id').using('MotorConstructionEquipment')
            assignedPerson = ServiceAssigned.objects.filter(ServiceId=seId[0]).values('To__UserName','To__RoleId__id').distinct().using('MotorConstructionEquipment')
        else :
        ...
        print('--Assign List--' + str(assignedPerson))
